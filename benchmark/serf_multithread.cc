/**
 * @file serf_multithread.cc
 * @author Chaoji Zuo (chaoji.zuo@rutgers.edu)
 * @brief SeRF Multi-threaded Query Benchmark
 * @date 2025-01-06
 *
...
void process_query_batch(
    const QueryBatch& batch,
    SeRF::IndexSegmentGraph2D* index,  // 移除 const
  ...
  BaseIndex::SearchInfo local_search_info(data_wrapper, &i_params, "SeRF_2D_MT", "benchmark");

  for (size_t idx = batch.start_idx; idx < batch.end_idx; idx++) {
    int one_id = data_wrapper->query_ids.at(idx);

    // Calculate query range (same for all queries in the same range group)
...
void run_multithreaded_queries(
    SeRF::IndexSegmentGraph2D* index,  // 移除 const
    const DataWrapper* data_wrapper,
    const BaseIndex::SearchParams& s_params,
    int num_threads,
  ...
  for (const auto& [range, query_indices] : queries_by_range) {
    ...
    for (int t = 0; t < num_threads; t++) {
      workers.emplace_back([&, t]() {
        ...
        BaseIndex::SearchInfo local_search_info(data_wrapper, &local_i_params, "SeRF_2D_MT", "benchmark");
...
int main(int argc, char** argv) {
  ...
  string version = "SeRF_MultiThread";
  ...
  int batch_size = 32;      // Queries per batch
  ...
  if (groundtruth_path != "") {
    ...
    cout << "========================================" << endl;
    cout << "Generating Groundtruth (MULTI-THREADED)" << endl;
    cout << "========================================" << endl;
    cout << "Using OpenMP for parallel groundtruth generation..." << endl;
    cout << endl;
  ...
  for (int current_search_ef : search_ef_list) {
    ...
    if (use_multithread) {
      run_multithreaded_queries(&index, &data_wrapper, s_params, num_threads, batch_size);
    } else {
